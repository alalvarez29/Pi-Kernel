...
int main(int RAJA_UNUSED_ARG(argc), char** RAJA_UNUSED_ARG(argv[]))
{
	...
	//RAJA OpenMP implementation
		...
		using REDUCE_POL1 = RAJA::omp_reduce;

		RAJA::ReduceSum <REDUCE_POL1, double> omp_pi(0.0);		
	...
	//RAJA CUDA implementation
		...
		using EXEC_POL2 = RAJA::cuda_exec<CUDA_BLOCK_SIZE>;
		using REDUCE_POL2 = RAJA::cuda_reduce;

		RAJA::ReduceSum <REDUCE_POL2, double> cuda_pi(0.0);
		
		double cu_totalTime;

		auto cu_t1 = std::chrono::high_resolution_clock::now();

		RAJA::forall <EXEC_POL2>(RAJA::RangeSegment(0, N), [=] RAJA_DEVICE (int i)
		{
			double x = (double(i) + 0.5) * dx;
			cuda_pi += dx / (1.0 + x * x);
		});
